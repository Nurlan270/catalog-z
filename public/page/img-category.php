...
<head>
    <?php
        ...
        $categoryName = $categoryName->fetch()['name'] ?? header('Location: /public/page/img-category.php?id=' . $_GET['id']);

//         Pagination
        $page = $_GET['page'] ?? 1;

        if ($page <= 0 || !is_numeric($page)): header('Location: /public/page/img-category.php?id=' . $_GET['id']);
        endif;

        $limit = 8;
        $offset = $limit * ($page - 1);
        // Get count
        $query = $db->pdo->prepare('SELECT COUNT(*) AS c FROM images WHERE categories LIKE ? AND visibility = 1');
        $query->execute(['%'.$_GET['id'].'%']);
        $cnt = $query->fetch()['c'];
        $total_pages = ceil($cnt / $limit);

        if ($page > $total_pages): header('Location: /public/page/img-category.php?id=' . $_GET['id']);
        endif;

        $images = $db->pdo->prepare("SELECT * FROM images WHERE visibility = ? AND categories LIKE ? ORDER BY id DESC LIMIT $limit OFFSET $offset");
        $images->execute([1, '%'.$_GET['id'].'%']);
    ...
    <div class="container-fluid tm-container-content tm-mt-60">
        <div class="row mb-4">
            ...
            <div class="col-6 d-flex justify-content-end align-items-center">
                <form method="get" action="img-category.php" class="tm-text-primary">
